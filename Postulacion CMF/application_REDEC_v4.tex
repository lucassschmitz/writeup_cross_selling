...
Paralelamente a la nómina de deudores, en Chile opera un segundo sistema de información crediticia: los registros comerciales administrados por distribuidores privados (como Equifax/DICOM) y el Boletín de Informaciones Comerciales de la Cámara de Comercio de Santiago, regulados por la Ley 19.628 sobre protección de la vida privada. A diferencia de la nómina --- que registra el stock completo de deudas vigentes y vencidas de las instituciones fiscalizadas ---, estos registros comerciales consignan principalmente eventos negativos: protestos, morosidades e incumplimientos.

En los últimos veinte años, el marco regulatorio chileno en materia de información crediticia ha experimentado múltiples reformas . Entre 2010 y 2012, tres leyes sucesivas (Leyes 20.455, 20.522 y 20.575) \textit{restringieron} la información disponible en los registros comerciales.\footnote{Estas leyes modificaron la Ley 19.628, que rige los registros de crédito comerciales (Boletín Comercial, DICOM/Equifax), y no el artículo 14 de la LGB que gobierna la nómina de deudores.} La Ley 20.455 suspendió la publicación de morosidades de desempleados de corto plazo; la Ley 20.522 excluyó las consultas crediticias del puntaje; y la Ley 20.575 eliminó historiales de impago por montos menores, afectando a 2,8 millones de personas \parencite{liberman_equilibrium_2018, madeira_impact_2024}. En dirección opuesta, reformas posteriores han \textit{ampliado} los sistemas de información crediticia. La Ley 21.130 de modernización bancaria habilitó legalmente en 2019 e incorporó operativamente entre 2021 y 2022 a los emisores de tarjetas de crédito no bancarios (ETNB) como reportantes y receptores de la nómina de deudores. La Ley 21.680, que crea el Registro de Deuda Consolidada (REDEC) y entra en vigencia el 1 de abril de 2026, expande significativamente el universo de entidades participantes. Finalmente, la Ley Fintec (Ley 21.521) establece un sistema de finanzas abiertas que permitirá el intercambio de datos financieros entre instituciones mediante APIs estandarizadas.
\begin{itemize}
...
El REDEC introduce dos cambios sustanciales respecto del sistema anterior. Primero, amplía el universo de entidades participantes. Bajo el artículo 14 LGB, solo las instituciones financieras fiscalizadas reportaban y recibían información consolidada. El REDEC extiende esta obligación a oferentes de crédito no bancarios (OCNB), incluyendo cajas de compensación y compañías de seguro, entre otros.\footnote{La lista completa de entidades reportantes se establece en el artículo 2, letra e) de la Ley 21.680. Además de cajas de compensación y compañías de seguro, incluye agentes administradores de mutuos hipotecarios endosables, sociedades securitizadoras, asesores crediticios regulados por la Ley Fintec, y acreedores no fiscalizados que superen ciertos umbrales de actividad.} Segundo, enriquece el contenido de la información reportada. Si bien la nómina de deudores del artículo 14 ya registraba el stock de deudas vigentes y vencidas de las instituciones fiscalizadas, el REDEC exige además reportar los principales términos y condiciones de los créditos, plazos, garantías constituidas y estado de cumplimiento (Art.\ 4, Ley 21.680). Esto permite a los competidores del incumbente no solo observar que un deudor tiene créditos vigentes, sino evaluar con mayor precisión su capacidad de pago --- que es precisamente el canal a través del cual el intercambio de información intensifica la competencia.
...
La estrategia de identificación explota el hecho de que el REDEC revela a los bancos información sobre la deuda de sus clientes con OCNB que antes era inobservable. La muestra principal para las regresiones en forma reducida es el conjunto de individuos que tenían créditos bancarios vigentes antes de la reforma y que además mantenían deuda con OCNB.\footnote{Identificamos a estos individuos retrospectivamente usando los datos de OCNB que el REDEC incorpora al sistema.} Antes de abril de 2026, los bancos observan el historial bancario de estos individuos pero no su deuda con OCNB. Después del REDEC, los bancos observan ambos. Definimos $Good_i^{OCNB} = \mathbf{1}[\text{sin morosidad en OCNB}]$ como indicador de buen historial crediticio en el sistema no bancario.\footnote{Para clasificar a los individuos podríamos usar técnicas de \textit{machine learning} siguiendo a \textcite{liberman_equilibrium_2018}.}

Estimamos tres especificaciones. La primera mide el efecto del REDEC sobre el acceso al crédito de bancos competidores:
\begin{equation}
    NuevoBanco_{it} = \beta^A \cdot (Post_t \times Good_i^{OCNB}) + \gamma X_{it} + \delta_i + \theta_t + \varepsilon_{it}
    \label{eqn:access}
\end{equation}
donde $NuevoBanco_{it}$ es un indicador de si el individuo $i$ obtiene un crédito de un banco distinto a su incumbente en el período $t$, $Post_t$ indica los períodos posteriores a la entrada en vigencia del REDEC (abril 2026), $X_{it}$ son controles individuales variables en el tiempo, $\delta_i$ son efectos fijos individuales y $\theta_t$ son efectos fijos de tiempo. La teoría predice $\hat{\beta}^A > 0$: una vez que el buen historial OCNB se vuelve visible, los bancos competidores pueden identificar a estos deudores como buenos tipos y hacerles ofertas.

La segunda especificación mide el efecto sobre las tasas de interés:
\begin{equation}
    r_{ijt} = \beta^P \cdot (Post_t \times Good_i^{OCNB}) + \gamma X_{it} + \delta_i + \mu_j + \theta_t + \varepsilon_{ijt}
    \label{eqn:pricing}
\end{equation}
donde $r_{ijt}$ es la tasa de interés del crédito otorgado al individuo $i$ por el acreedor $j$ en el período $t$, $\delta_i$ son efectos fijos individuales y $\mu_j$ son efectos fijos de acreedor. La teoría predice $\hat{\beta}^P < 0$: al observar que el deudor tiene buen historial con OCNB, los bancos competidores ofrecen tasas más bajas para atraerlo.
...
El argumento de identificación en las ecuaciones (\ref{eqn:access}) y (\ref{eqn:pricing}) descansa en que, antes del REDEC, la variable $Good_i^{OCNB}$ --- basada en el historial con OCNB --- es inobservable para los bancos, por lo que no debería afectar diferencialmente las condiciones de crédito bancario. Esto provee un test placebo natural: esperamos que los coeficientes de $Post_t \times Good_i^{OCNB}$ para períodos anteriores a abril de 2026 no sean estadísticamente significativos.
