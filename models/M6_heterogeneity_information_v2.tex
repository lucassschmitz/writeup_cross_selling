...
\textbf{Parameters.} The simulation parameters are set in \texttt{Code/model6/params.m}, which serves as the single source of truth for both simulation and estimation.
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lll}
\toprule
...
\midrule
Price sensitivity & $\alpha$ & Logit demand coefficient \\
Switching cost & $\lambda$ & In rate units \\
Grid sizes & $N_x, N_h$ & Public covariate and type grids \\
Support & $[h_{\min}, h_{\max}]$ & Default probability range \\
 & $[x_{\min}, x_{\max}]$ & Public covariate range \\
Joint distribution & $\mathbf{P}$ & $N_x \times N_h$ matrix; see code for construction \\
Sample size & $N$ & Number of simulated borrowers \\
\bottomrule
\end{tabular}
\end{center}
