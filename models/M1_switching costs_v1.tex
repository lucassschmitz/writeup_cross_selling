...
\textcolor{red}{For this simulation I should determine 1. whether prices are at the consumer or market level. currently i am assuming they are at the market level, which I do not think is the correct assumption. 2. when prices are at the consumer level, what are exactly the variables that the firm observes when pricing their products. 3. in BLP there are many markets, here do we have only one market and price heterogeneity within that market?  }
...
\end{align}
This formulation is differentiable in prices and is preferable for the equilibrium computation. The $\epsilon_{ijt}$ draws are then only needed in Step 3 for computing realized outcomes.

\textbf{Remark:}  Because $\mathcal{S}_k$ is a \emph{selected} sample, consumers in $\mathcal{S}_k$ tend to have high $\mu_{ik}$. This selection, combined with the switching cost $s$, generates stronger lock-in than either force alone.
...
\end{enumerate}
Note: the standard logit markup formula $p = c + 1/[\alpha(1-D)]$ does \emph{not} apply here because demand is a mixed logit (integrated over the selected $\mu$ distribution), so we use numerical optimization instead.

\subsection{Step 2: Period 1 Equilibrium}

The key complication relative to the plain logit model is that the Period 2 equilibrium---and hence $\hat{\pi}_{2j}(k)$---depends on $p_1$ through the composition of the pools $\mathcal{S}_k(p_1)$. The firm's total profit is:
\begin{align}
    \hat{\Pi}_j(p_1) = \frac{|\mathcal{S}_j(p_1)|}{N}(p_{j1} - c_1) + \sum_{k=1}^{2} \frac{|\mathcal{S}_k(p_1)|}{N} \hat{\pi}_{2j}(k; p_1)
\end{align}
where $\hat{\pi}_{2j}(k; p_1)$ is the Period 2 equilibrium profit of firm $j$ from pool $k$, which must be \emph{re-solved} for each candidate $p_1$.
...
\textbf{Remark (computational cost):} The nested structure (Period 2 equilibrium inside Period 1 search) is expensive because each evaluation of $\hat{\Pi}_j(p_1)$ requires a full Period 2 equilibrium solve. Two modifications can dramatically reduce the cost:

\begin{enumerate}
    \item \textbf{Smooth pool assignments.} The hard partition $\mathcal{S}_k(p_1)$ via $\arg\max$ makes pool sizes jump discretely as $p_1$ changes. Instead, replace the indicator $\mathbb{I}[i \in \mathcal{S}_k]$ with the logit probability that consumer $i$ chose firm $k$:
    \begin{align}
        w_{ik}(p_1) = \frac{\exp(\delta_{k1} + \mu_{ik})}{\sum_{j'} \exp(\delta_{j'1} + \mu_{ij'})}
    \end{align}
    This integrates out $\epsilon_{i1}$ analytically. The smoothed Period 2 demand from Step 1 becomes:
    \begin{align}
        \hat{D}_{2j}^{\text{smooth}}(k, p_2(k); p_1) = \frac{\sum_{i=1}^N w_{ik}(p_1) \cdot \hat{D}_{2j}^{i}(k, p_2(k))}{\sum_{i=1}^N w_{ik}(p_1)}
    \end{align}
    where $\hat{D}_{2j}^{i}$ is the same per-consumer logit probability from Step 1. Everything is now smooth in $p_1$.
    
    \item \textbf{Solve all FOCs simultaneously.} Instead of nesting the Period 2 equilibrium inside the Period 1 search, stack all $2 + 2 \times J$ prices into a single vector $\mathbf{p} = (p_{11}, p_{21}, p_{12}(1), p_{22}(1), p_{12}(2), p_{22}(2)) \in \mathbb{R}^6$ and solve the system of 6 FOCs simultaneously using a nonlinear equation solver (\texttt{fsolve}). This eliminates the inner loop entirely: each Jacobian evaluation is a single pass over $N$ consumers, and the solver typically converges in $<20$ Newton steps.
\end{enumerate}

These two modifications can be applied independently or together. Modification 1 alone makes the nested algorithm smoother and more robust. Modification 2 alone eliminates the nesting. Together, they yield a single smooth $6 \times 6$ system that is typically orders of magnitude faster than the nested approach.
